...
El importador organiza los valores como tokens `base` y añade aliases hacia los nombres normalizados: `color.bg`, `color.surface`, `color.text`, `color.action`, `color.border`, `space.4` y `typography.font-family-base`. El CSS descargable consume estos nombres normalizados, no variables paralelas de importación. El resultado es una base técnica, no un sistema publicado: se debe revisar su intención semántica, cobertura white label, contrastes WCAG y componentes afectados antes de incorporarlo a [01-tokens/tokens.dtcg.json](01-tokens/tokens.dtcg.json) o al [05-agentes/component-manifest.json](05-agentes/component-manifest.json).
